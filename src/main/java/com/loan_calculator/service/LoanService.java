...
public class LoanService {
    ...
    public LoanResponseDTO createLoan(CreateLoanRequestDTO createLoanRequestDTO) {
        ...
        final LoanRequest loanRequest = loanRequestMapper.toEntity(createLoanRequestDTO);
        loanRequest.setStatus(LoanStatus.CREATED);

        CompletableFuture<List<Installment>> calculationResultFuture = loanCalculation.async_calculateInstallments(loanRequest);
        calculationResultFuture.thenAccept(installments -> {
            processInstallments(installments, loanRequest);
        });

        LoanRequest savedLoanRequest;
        try {
            savedLoanRequest = loanRequestRepository.saveAndFlush(loanRequest);
        ...
        return loanRequestMapper.toResponseDTO(savedLoanRequest);
    }

    private void processInstallments(List<Installment> installments, LoanRequest loanRequest) {
        loanRequest.setInstallments(installments);
        loanRequest.setStatus(LoanStatus.DONE);
